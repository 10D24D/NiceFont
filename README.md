...
- **灵活配置**：保存你的字体设置，轻松应用到每个网页。支持首次或动态调整网页字体，快速对本站、域名、所有网站进行设置。 
...
- **✏️ 字体描边**：可调节描边粗细，提升文字边缘清晰度。
- **🌑 字体阴影**:  可调节阴影强度，增强层次感。
- **📏 字体大小调整**：一键放大/缩小字体，并显示当前调整情况。
- **♻️ 恢复字体**：一键恢复字体，还原网页原始字体大小和风格。
- **1️⃣ 首次调整**：页面加载后自动应用字体设置，支持自定义延迟时间。
- **🔎 动态调整**：页面内容变化时自动应用字体设置，适合实时变动的网页（需注意复杂页面可能会出现卡顿现象，可搭配排除元素功能进行优化）。
- **🚫 排除元素**：支持排除自定义 CSS 选择器对应的网页元素。
- **🎨 切换面板**: 萝卜青菜各有所爱，可根据个人喜好使用面板。
- **📍 当前配置范围**：支持排除站点，或匹配本站、匹配域名及匹配所有网站进行字体调整（根据不同情况，点击操作可删除对应配置）。
...
1. 安装 [Tampermonkey](https://www.tampermonkey.net/) 或 [Violentmonkey](https://violentmonkey.github.io//) 或 [脚本猫](https://docs.scriptcat.org/)🛠️。
2. 访问 [NiceFont Github资源](https://github.com/10D24D/NiceFont/raw/refs/heads/main/NiceFont.user.js) 或 [NiceFont 油叉资源](https://greasyfork.org/zh-CN/scripts/533232-nicefont) 或 [NiceFont 脚本猫资源](https://scriptcat.org/zh-CN/script-show-page/5705)🌐。
...
- **修复**：
  - 解决字体应用策略升级后，Shadow-root DOM（例如B站评论区）内字体大小无法调整的问题。

### v4.3.1 - 2026-03-26

- **修复**：
  - 修复配置范围为“匹配所有网站”时，保存配置失败的BUG。
  - 修复调整字体大小时，字体家族被覆盖的BUG。
- **改进**：
  - 修改配置范围相关文案，减少文案歧义。
  - 决定移除“定时调整”功能，自动变更为“动态调整”功能，已兼容原有配置。
  - 保存前新增模式校验提示：未启用首次/动态时，先引导选择后再保存，避免配置保存后不生效。
...
- **首次调整、动态调整该怎么选？**  
  一般按网页变化频率选择：页面基本不变时用“首次调整”；论流、聊天、在线文档这类持续变化页面用“动态调整”。  
  如果你不确定，建议先开“首次调整”，最后才用“动态调整”（实时性最好，但性能开销也比较大）。
- **遇到问题怎么办？**  
...
<table>
  <tr>
    <td align="center">
      <img src="https://raw.githubusercontent.com/10D24D/NiceFont/refs/heads/main/static/wxpay.jpg" width="180"/><br/>
      <sub>微信支付</sub>
    </td>
    <td align="center">
      <img src="https://raw.githubusercontent.com/10D24D/NiceFont/refs/heads/main/static/alipay.jpg" width="180"/><br/>
      <sub>支付宝</sub>
    </td>
  </tr>
</table>